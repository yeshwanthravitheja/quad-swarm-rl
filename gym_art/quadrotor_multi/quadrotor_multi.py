...
class QuadrotorEnvMulti(gym.Env):
    def __init__(self, num_agents, ep_time, rew_coeff, obs_repr,
                 ...
                 # Obstacle
                 use_obstacles, obst_density, obst_size, obst_spawn_area, obst_obs_type, obst_noise,
    ...
    def reset(self, obst_density=None, obst_size=None):
        ...
        # Scenario reset
        if self.use_obstacles:
            self.obstacles = MultiObstacles(obstacle_size=self.obst_size, quad_radius=self.quad_arm, obs_type=self.obst_obs_type, noise=obst_noise)
